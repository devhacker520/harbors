...
exports.createServer = function(config,hostList){
    ...
    http.createServer(function(req,res){
        ...
        //获取数据
        _sync('filter')
            .filter(function(callback){//整理cookie数据
            ...
            .add(function(callback){//获取post数据以及文件
                if(req.method=='POST'){
                    //检查文件夹是否存在
                    if(!fs.existsSync(config.baseDir+'/tmp'))
                        fs.mkdirSync(config.baseDir+'/tmp');
                    var form = new formidable.IncomingForm();
                    form.uploadDir = config.baseDir+'/tmp'
            ...
            .end(function(){
                ...
                try{
                    require(config.baseDir+'/host/'+hostList[vhost].serverDir+'/controller/filter')($object);
