...
function setSession(route,num,req,res,config){
    res.setSession = function(session,expire){
        if(expire==0&&req.cookie[config.sessionId]){
            ...
            eval('redis.hdel("'+req.cookie[config.sessionId]+'"'+s+')');
            return false;
        }
        if(req.cookie[config.sessionId]&&req.session!=null){
            redis.hmset(req.cookie[config.sessionId], session);
            //过期时间
            redis.expire(req.cookie[config.sessionId], expire||config.SessionSurvavil/1000);
        }else{
            ...
            //过期时间
            redis.expire(a, expire||config.SessionSurvavil/1000);
        }
    };
    route[++num](route,num,req,res,config);
}
