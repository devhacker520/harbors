...
function setSession(route,num,req,res,config){
    res.setSession = function(session,expire){
        ...
        if(expire==0&&req.cookie[config.sessionId]){
            ...
            _redis.set(req.cookie[config.sessionId], harbors.utils.o2s(req.session));
            return false;
        }
        if(req.cookie[config.sessionId]&&req.session!=null){
            _redis.set(req.cookie[config.sessionId], harbors.utils.o2s(session));
            //过期时间
            ...
            res.setCookie(b);
            _redis.set(a, harbors.utils.o2s(session));
            //过期时间
