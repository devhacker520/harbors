...
function setSession(route,num,req,res,config){
    res.setSession = function(session){
        if(req.cookie[config.sessionId]){
            ...
            //过期时间
            redis.expire(req.cookie[config.sessionId], config.SessionSurvavil/60);
        }else{
            ...
            //过期时间
            redis.expire(a, config.SessionSurvavil/60);
        }
    };
    route[++num](route,num,req,res,config);
}
...
function mysql(route,num,req,res,config){
    res.mysql = function(query,callback){
        _mysql.query(query,function(error, results, fields){
