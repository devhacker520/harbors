...
var app = {
    ...
    render:function(file){
        var file = file;
        if(file.slice(-1) === "/") {
            file = file + harbors.config.sraticFile;
        }else if(!/\./.test(file)) {
            file = path.join(file, "/", harbors.config.sraticFile);
        };
        ...
        fs.exists(realPath,function(exists){
            if (!exists) {
                ...
                fs.stat(realPath, function (err, stat) {
                    ...
                    _res.setHeader("Last-Modified", lastModified);

                    if (_req.headers[ifModifiedSince] && lastModified == _req.headers[ifModifiedSince]) {
                        _res.writeHead(304, "Not Modified");
                        _res.end();
                    } else {
