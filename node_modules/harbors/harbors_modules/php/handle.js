...
module.exports = function (object, realFile) {
    ...
    var params = _utils.makeHeaders(object.client.headers, [
        ...
        ["SCRIPT_NAME", object._path.rewrite || object._path.pathname],
        ...
        ["PHP_SELF", object._path.rewrite || object._path.pathname],
    ...
    connection.addListener("connect", function () {
        ...
        var body = new Buffer(64*1024);
        var j = 0;

        parser.onRecord = function (record) {
            if (record.header.type == FCGI_STDOUT) {
                //body += record.body;
                for(i=0; i < record.body.length; i++) {
                    body[j++] = record.body[i];
                }
                ...
                var _body = body.slice(0,j).toString();
                var parts = _body.split("\r\n\r\n");

                var headers = parts.shift();
                var headerParts = headers.split("\r\n");

                body = body.slice(headers.length+4,j);
