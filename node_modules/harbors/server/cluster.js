...
var createWorker = function(){//创建工作进程
    ...
    harbors._dev&&cp.send({type:'dev'});
    cp.on('message',function(a){
        if(a['type']){
            switch(a.type){
                case 'session':
                    Session.master(cp,a);
...
if (cluster.isMaster) {
    ...
    process.on('message',function(a){
        if(a['type']){
            switch(a.type){
                ...
                case 'session':
                    console.log(a)
                    if(a.info.session){//返回session存在，请求是查询请求

                    }else{//返回session不存在，刚刚完成的插入新session请求
    ...
    http.createServer(function(req,res){
        ...
        if(!req.vhost.type){
            ...
            harbors._dev&&console.log('vhost: '+req.vhost.baseDir +', type: '+req.vhost.type);
