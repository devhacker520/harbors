...
exports.write = function(data){
    try{
        ...
        //检测创建文件夹
        __fs__.mkdir(data.dir);
        //读取文件夹内的日志文件
        var files = __fs__.readDir(data.dir);
        //过滤无用的文件
        ...
        if(files.length==0){
            ...
            __fs__.writeFile(path.join(data.dir,name),logC);
            return;
        }
        //非空目录判断日志文件个数，日志大小
        files = files.sort();
        ...
        var stat = __fs__.stat(path.join(data.dir,files));
        if(stat.size>data.size*1024){//超出大小新建文件
            ...
            __fs__.appendFile(path.join(data.dir,files),logC,'utf8',function(err){
                if(err){
                    console.log(err);
                }
            });
